...
class Deliver():
    ...
    # __str__n
    def getInfo(self):
        return f"""\nID: {self._code_id}\nFecha: {self._date}\nHora: {self._time}\nSender: {self._sender.getInfo()}\nReciver: {self._recive.getInfo()}\nAdress: {self._reciver_add_address.getInfo()}\nContacto:  {self._contact.getInfo()}\nItems: {self._items.getInfo()}  """


objeto = Deliver(1, 2, 3, Person(1, "samuel", "tobio", "19"), Person(2, "mariana", "ortiz", "20"), Address("Calle 2", "La princesa",
                 "Department 6"), Address("Calle3", "La Princesa", "Department 7"), Person(3, "Juan", "Perez", "20"), Package(4, 68.3, "Big Package"))
print(objeto.getInfo())
